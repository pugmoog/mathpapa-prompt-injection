javascript:(function () {
  ...
  function updateEntry(entry, rawText) {
    if (rawText !== entry.lastText) {
      ...
      entry.thinkBody.textContent = parts.reasoning;

      if (parts.found && !entry.resolved) {
        entry.resolved = true;
  ...
  function finishInitialEntry(entry, rawText) {
    ...
    if (!entry.resolved) resolveEntryAsPlain(entry, rawText);
  }
  function send() {
    var q = input.value.trim();
    if (!q || pendingRequests) return;
    addUserBubble(q);
    input.value = '';
    var entry = makeEntry();
